...
- Strategic economy second-act groundwork is in progress on `feature/strategic-economy-second-act`: VenaTerra now exposes a read-only deposit survey API in `/home/kaiserroman/venaterra`, BannerMod has an optional reflection bridge, claim-level food/iron/wood/stone/coin summary hints, strategic mine site records, coarse mine yield hints, and admin debug commands for `/bannermod debug economy claim <claimUuid>` plus `/bannermod debug economy mines <claimUuid>`. See `docs/STRATEGIC_ECONOMY_INTEGRATION.md`.
...
- Strategic mine output is currently a summary/read-model hint only. It does not yet mutate storage, treasury, or item stacks; mine maintenance/depletion (`ECON-024`), fort caps (`ECON-025`), NPC contracts (`ECON-029`), and strategic resource debit/credit (`ECON-031`) remain open.
